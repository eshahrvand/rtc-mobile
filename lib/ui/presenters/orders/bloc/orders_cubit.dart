    ...
    return _ordersRepo
        ...
        .then((html) async {
          ...
          if (generatedFile != null) {
            ...
            // We switch to saveAs because on Android 11+, saveFile is restricted to
            // app-specific private directories (Android/data/...), making files invisible
            // in the Downloads app. saveAs invokes the system picker for public folders.
            ...
            debugPrint('>> [PDF] FileSaver.saveAs result: $savedPath');
            ...
            if (savedPath != null && savedPath.isNotEmpty) {
              emit(
                state.copyWith(
                  ...
                  // Use the temporary path for sharing to ensure maximum compatibility
                  lastPrintedFilePath: generatedFile.path,
                ),
              );
            } else {
              // User likely cancelled the "Save As" dialog
              emit(state.copyWith(isPrinting: false));
            }
          } else {
            debugPrint('>> [PDF] ERROR: generatedFile was null');
            dev.log('PDF generation failed', name: 'PDF_DEBUG', error: 'generatedFile is null');
          }
